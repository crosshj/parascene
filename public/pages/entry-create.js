...
function runCreatePageInit() {
	...
	function saveStyleIndex(index) {
		try {
			localStorage.setItem(STORAGE_KEYS.styleIndex, String(Math.max(0, index)));
		} catch (_) {}
	}
	function saveStyleSelected(key) {
		try {
			localStorage.setItem(STORAGE_KEYS.styleSelected, String(key || ''));
		} catch (_) {}
	}
	...
	if (textToImagePrompt) {
		const v = (() => {
			try {
				return localStorage.getItem(STORAGE_KEYS.promptText);
			} catch (_) {
				return null;
			}
		})();
	...
	if (imageEditPrompt) {
		const v = (() => {
			try {
				return localStorage.getItem(STORAGE_KEYS.promptImageEdit);
			} catch (_) {
				return null;
			}
		})();
	...
	if (allStyleCards?.length) {
		...
		(async () => {
			const { getStyleThumbUrl } = await import('./create-styles.js');
	...
	if (tabsEl) {
		tabsEl.addEventListener('tab-change', (e) => {
			if (e.detail?.id) saveTab(e.detail.id);
		});
	}
	...
	window.updateEditImageButtonState = function updateEditImageButtonState() {
	...
	void loadMutateOptions().then(() => window.updateEditImageButtonState?.());
	if (imageEditPrompt) {
		imageEditPrompt.addEventListener('input', window.updateEditImageButtonState);
		imageEditPrompt.addEventListener('change', window.updateEditImageButtonState);
	}
