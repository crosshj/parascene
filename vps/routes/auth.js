...
async function createSession(db, res, userId) { const token = issueToken(userId); await db.createSession(userId, hashToken(token), new Date(Date.now() + SESSION_MS).toISOString()); setSessionCookie(res, token); }
