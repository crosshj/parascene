...
export function createDb() {
	...
	return {
		async userByEmail(email) { const { data, error } = await client.from(table("users")).select("id, email, password_hash, role, created_at, last_active_at, meta").ilike("email", email).maybeSingle(); if (error) throw error; return data || null; },
		async userByUsername(username) {
			const { data: profile, error: profileError } = await client.from(table("user_profiles")).select("user_id").ilike("user_name", username).maybeSingle();
			if (profileError) throw profileError;
			if (!profile?.user_id) return null;
			return this.userByIdForLogin(profile.user_id);
		},
		async userByIdForLogin(id) { const { data, error } = await client.from(table("users")).select("id, email, password_hash, role, created_at, last_active_at, meta").eq("id", id).maybeSingle(); if (error) throw error; return data || null; },
