...
import createSunoRoutes from "../api_routes/suno.js";
...
import createChallengesRoutes from "../api_routes/challenges.js";
...
import createGroupV2Routes from "../api_routes/group_v2.js";
import createQRRoutes from "../api_routes/qr.js";
import createKioskRoutes from "../api_routes/kiosk.js";
...
// store JSON generated by scripts/analytics/overview-refresh.js.
if (process.env.NODE_ENV !== "production" && !process.env.VERCEL) {
	const reportsAppDir = path.join(__dirname, "..", "scripts", "analytics", "overview");
	const reportsCssFile = path.join(__dirname, "..", "scripts", "analytics", "report.css");
	const reportsStoreFile = path.join(__dirname, "..", ".output", "overview", "store.json");
	app.get("/reports/store.json", (req, res) => res.sendFile(reportsStoreFile));
	app.get("/reports/report.css", (req, res) => res.sendFile(reportsCssFile));
	app.use("/reports", express.static(reportsAppDir, { index: "index.html" }));
}
...
app.use(createAudioClipsRoutes({ queries, storage }));
app.use(createLibraryFoldersRoutes({ queries }));
app.use(createGroupV2Routes({ queries }));
...
app.use(createChallengesRoutes({ queries }));
app.use(createShareRoutes({ queries, storage }));
app.use(createQRRoutes());
app.use(createKioskRoutes({ queries }));
