...
/** Prompt injection tags: slug + tag_type → injection_text; sigil from tag_type (see db/schemas/sqlite_03_prompt_injections.sql). */
function ensurePromptInjectionsTable(db) {
	try {
		db.exec(`
		...
		`);
		db.exec("DROP INDEX IF EXISTS idx_prompt_injections_tag_lower");
		db.exec("DROP INDEX IF EXISTS idx_prompt_injections_tag_global_unique");
		db.exec("DROP INDEX IF EXISTS idx_prompt_injections_tag_user_unique");
		db.exec(`
