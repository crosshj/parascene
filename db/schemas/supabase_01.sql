...
ALTER TABLE prsn_provider_registry ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE prsn_provider_registry IS 'Parascene: registry of provider servers and their contact information. RLS enabled without policies - only service role can access. All access controlled via API layer.';

ALTER TABLE prsn_provider_statuses ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE prsn_provider_statuses IS 'Parascene: current status and health metrics for provider servers. RLS enabled without policies - only service role can access. All access controlled via API layer.';

ALTER TABLE prsn_provider_metrics ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE prsn_provider_metrics IS 'Parascene: performance metrics and statistics for provider servers. RLS enabled without policies - only service role can access. All access controlled via API layer.';

ALTER TABLE prsn_provider_grants ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE prsn_provider_grants IS 'Parascene: grants and funding information for provider servers. RLS enabled without policies - only service role can access. All access controlled via API layer.';

ALTER TABLE prsn_provider_templates ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE prsn_provider_templates IS 'Parascene: template catalog for provider servers. RLS enabled without policies - only service role can access. All access controlled via API layer.';
...
COMMENT ON TABLE prsn_servers IS 'Parascene: community servers and their metadata. RLS enabled without policies - only service role can access. All access controlled via API layer.';
