...
export function mountSequentialVideoPlayer(container, slides, options = {}) {
	...
	const crossfadeEnabled = len > 1 && options.crossfade !== false;
	let crossfadeMs = Number(options.crossfadeMs);
	if (!Number.isFinite(crossfadeMs)) crossfadeMs = crossfadeEnabled ? 100 : 0;
	let overlapSec = Number(options.overlapSec);
	if (!Number.isFinite(overlapSec)) overlapSec = crossfadeEnabled ? 0.1 : 0;
	if (!crossfadeEnabled) {
		crossfadeMs = 0;
		overlapSec = 0;
	}
	...
	const attachOverlapAdvance = (video) => {
		if (
			len <= 1 ||
			overlapSec <= 0 ||
			!autoAdvanceOnEnded ||
		...
		const onTimeUpdate = () => {
			if (tornDown || advanceLock || userPaused) return;
			if (video !== videos[activePlayer]) return;
			const d = video.duration;
			const remain = d - video.currentTime;
			if (!Number.isFinite(d) || d <= 0 || !Number.isFinite(remain)) return;
			if (remain > overlapSec) return;
	...
	const crossfadeToPlayer = async (incomingPlayer, outgoingPlayer, slide) => {
		...
		const useCrossfade = crossfadeMs > 0 && len > 1;
		...
		if (useCrossfade) {
			slot.classList.add('sequential-video-slot--crossfading');
			slot.style.setProperty('--sequential-video-crossfade-ms', `${crossfadeMs}ms`);
			incoming.classList.add('sequential-video-player-video--crossfade-in');
			if (outgoing instanceof HTMLVideoElement) {
				outgoing.classList.add('sequential-video-player-video--crossfade-out');
			}
		}
		...
		const handoff = () => {
			...
			slot.classList.remove('sequential-video-slot--crossfading', 'sequential-video-slot--crossfade-run');
			slot.style.removeProperty('--sequential-video-crossfade-ms');
			incoming.classList.remove('sequential-video-player-video--crossfade-in');
			if (outgoing instanceof HTMLVideoElement && outgoing !== incoming) {
				outgoing.pause();
				outgoing.classList.remove('is-active', 'sequential-video-player-video--crossfade-out');
		...
		if (useCrossfade) {
			requestAnimationFrame(() => {
				requestAnimationFrame(() => {
					slot.classList.add('sequential-video-slot--crossfade-run');
				});
			});
			crossfadeTimer = window.setTimeout(handoff, crossfadeMs);
			...
			incoming.addEventListener('error', handoff, { once: true });
			return;
		}
	...
	const onVideoEnded = (e) => {
		if (userPaused || loopSingleClip || !autoAdvanceOnEnded || advanceLock || tornDown) return;
